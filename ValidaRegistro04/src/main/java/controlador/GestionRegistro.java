...
public class GestionRegistro {
    ...
    public ListaSolicitudes leerArchivoSerializado(File archivoSerializado)throws IOException, ClassNotFoundException{
        try(ObjectInputStream ois = new ObjectInputStream(new FileInputStream(archivoSerializado))){
            return (ListaSolicitudes) ois.readObject();
        }catch (IOException ioe){
            throw new IOException("Error en la lectura del archivo serializado",ioe);
        }catch(ClassNotFoundException clne){
            throw new ClassNotFoundException("Error en la lectura del archivo serializado",clne);
        }
    }
