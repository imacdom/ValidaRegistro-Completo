...
public class Programa {
    ...
    public void ejecutar() throws IOException, ClassNotFoundException {
        ...
        utilES.mostrarMensajeln("\n---MOSTRAR REGISTRO SERIALIZADO---");

        ListaSolicitudes solicitudesSerializadas;
        solicitudesSerializadas= gr.leerArchivoSerializado(archivo);
        mostrarSolicitudes(solicitudesSerializadas);


        gr.serializarRegistro(archivo);
    }


    private void mostrarSolicitudes(ListaSolicitudes solicitudes){
        for(Solicitud solicitud: solicitudes){
            utilES.mostrarMensajeln(solicitud.toString());
        }
    }
